...
## Check for reasons why data not in Marvid
```{r}
refer %>% filter(is.na(total_records))%>%
  select(nRefStation,nSampleNumber,Comments,Other.comments,Notes,Notes.II) %>%
  addHtmlTableStyle(css.cell = c("width: 50;","width: 50;","width: 50;","width: 50;","width: 50;","width: 50;")) %>%
  htmlTable()
```
