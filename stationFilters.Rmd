...
dim(refer)[1]
```
### Last cruise included in this lot
```{r}
max(refer$cruise_no)
```

## List stations whose data is not in MarVid
Check for reasons why. Keep in mind that it's possible the station has not yet been analyzed.
```{r}
...
#### Length filter fail
```{r}
sample_info_f %>% filter(SegLengthM<length_lower|SegLengthM>length_upper)
```
